...
const AdminAttractionsPage: React.FC = () => {
    ...
    useEffect(() => {
        fetch('/api/admin-get-attractions')
            .then(res => res.json())
            .then(data => {
    ...
    const handleApprove = async (_id: string) => {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_SERVER_URL}/admin/approveAttraction`, {
            method: "POST",
            body: formData,
            credentials: "include",
        });
    ...
    const handleDisapprove = async (_id: string) => {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_SERVER_URL}/admin/disapproveAttraction`, {
            method: "POST",
            body: formData,
            credentials: "include",
        });
                ...
                {!loading && (
                    <div className="grid grid-cols-1 gap-10 items-start">
                        ...
                        <div className="space-y-5">
                            ...
                            {(attractions && attractions.length>0) ? (attractions.map((item, index) => (
                                <div key={index} className="bg-white p-5 rounded-2xl border flex gap-5 shadow-sm transition-all border-slate-200">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="flex gap-4 mt-4 pt-3 border-t border-slate-50">
                                            ...
                                            <Link
                                            href={`/attractions/preview?id=${item._id}`}
                            ...
                            <div className="bg-white border-2 border-dashed border-slate-200 rounded-3xl p-16 text-center">
                                <h3 className="text-xl font-bold text-slate-700">No attractions found</h3>
                            </div>
                            )}
                        </div>
