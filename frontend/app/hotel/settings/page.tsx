...
const HotelSettingsPage: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if(password.length || newPassword.length || confirmPassword.length) {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_SERVER_URL}/hotel/settings`, {
                method: "POST",
                headers: {
            ...
            router.push("/hotel/profile?successmessage=Successfully settings updated");
            return;
        }
        
        router.push("/hotel/profile?warnmessage=Nothing changed");
    }

    return (
        <div className="bg-indigo-50 min-h-screen w-full flex">
            <SidebarHotel pagetype="Settings" />
